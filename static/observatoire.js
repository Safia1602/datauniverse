document.addEventListener("DOMContentLoaded", () => {
  ...
  const TRACKED_TECH = [
    ...
    "looker studio", "snowflake", "bigquery", "redshift", "spark", "hadoop",
    "airflow", "dbt", "aws", "gcp", "azure", "saas", "llm", "generative ai",
    "machine learning",
  ];

  const MMW_SKILLS = [
    "sql", "python", "tableau", "power bi", "excel", "aws", "gcp", "snowflake",
    "dbt", "airflow",
  ];
  ...
  function classifyRole(titleRaw) {
    ...
    if (t.includes("business intelligence") || t.includes("bi ")) return "BI / Analytics";
    if (t.includes("machine learning") || t.includes("ml ")) return "ML Engineer";
    return "Other data role";
  }

  /* SKILL PARSER ROBUSTE */
  ...
  function rowParser(d) {
    ...
    return {
      ...
      _salary: salary,
      _exp: expYears,
      _salaryCurrency: (d.salary_currency || "").toUpperCase(),
      _salaryType: (d.salary_type || "").toLowerCase()
    };
  }
  ...
  fetch(DATA_FILE)
    ...
    .then((raw) => {
      ...
      // On ne filtre plus agressivement les dates. On garde tout ce qu'on a.
      allJobs = dataRaw; 
      
      console.log("Observatory data loaded:", allJobs.length);
      
      // Debug pour vérifier le format de date
      if(allJobs.length > 0) {
          console.log("Exemple de job (raw date):", raw[0].date_posted);
          console.log("Exemple de job (parsed date):", allJobs[0]._date);
      }
  ...
  function initFilters(data) {
    ...
    createCheckboxList(filterGroups.country, getUniqueValues(data, (d) => d._country));
    createCheckboxList(filterGroups.role, getUniqueValues(data, (d) => d._role));
    createCheckboxList(filterGroups.seniority, getUniqueValues(data, (d) => d.seniority_level || "Not specified"));
  ...
  function createCheckboxList(container, values) {
    ...
    container.innerHTML = "";
    if (!values.length) {
      container.innerHTML = '<em style="color:var(--muted); font-size:0.8rem;">No data</em>';
      return;
    }
    values.forEach((val) => {
  ...
  function initRoleSelects(data) {
    ...
    const roles = Array.from(new Set(data.map((d) => d._role).filter(Boolean))).sort();
  ...
  function setupFilterListeners() {
    ...
    [filterGroups.country, filterGroups.role, filterGroups.seniority].forEach((group) => {
      if (!group) return;
    ...
    if (resetFiltersBtn) {
      resetFiltersBtn.addEventListener("click", () => {
        ...
        if (filterGroups.skillsChips) {
          filterGroups.skillsChips.querySelectorAll(".chip").forEach((chip) => chip.classList.remove("active"));
        }
  ...
  function applyFilters() {
    ...
    // Calcul de 'now' seulement si des dates existent
    const now = d3.max(allJobs, (d) => d._date);
    let minDateFilter = null;
    
    // Si on a des dates et qu'un range est sélectionné
    if (now && dateRange === "30") {
    ...
    filteredJobs = allJobs.filter((job) => {
      // 1. Date (Seulement si un filtre date est actif ET que le job a une date)
      // Si le job n'a pas de date mais qu'on demande "Last 30 days", on l'exclut.
      if (minDateFilter) {
          if (!job._date || job._date < minDateFilter) return false;
      }

      // 2. Text Search
      if (text) {
        const t = (job.title || "").toLowerCase() + " " + (job.description_sans_html || job.description || "").toLowerCase();
        if (!t.includes(text)) return false;
      }

      // 3. Salary
      if (minSalary > 0) {
        const isUSD = job._salaryCurrency === 'USD' || job._salaryCurrency === '$';
        const isAnnual = job._salaryType.includes('annual') || job._salaryType.includes('year');
        if (!isUSD || !isAnnual || !job._salary || job._salary < minSalary) return false;
      }
      ...
      // 5. Checkboxes
      if (selectedCountries.length && !selectedCountries.includes(job._country)) return false;
      if (selectedRoles.length && !selectedRoles.includes(job._role)) return false;
      
      const sLevel = job.seniority_level || "Not specified";
      if (selectedSeniorities.length && !selectedSeniorities.includes(sLevel)) return false;
  ...
  function computeGlobalKPIs() {
    ...
    if (!data.length) {
        if(document.getElementById("kpi-total-jobs")) document.getElementById("kpi-total-jobs").textContent = "0";
        if(document.getElementById("kpi-hottest-role")) document.getElementById("kpi-hottest-role").textContent = "-";
        if(document.getElementById("kpi-hottest-tech")) document.getElementById("kpi-hottest-tech").textContent = "-";
        return;
    }

    // Gestion du cas où il n'y a PAS de dates du tout
    const now = d3.max(data, (d) => d._date);
    let last30 = data;
    let prev30 = [];
    
    if (now) {
        last30 = data.filter((d) => d._date && now - d._date <= 30 * 24 * 3600 * 1000);
        prev30 = data.filter((d) => d._date && now - d._date > 30 * 24 * 3600 * 1000 && now - d._date <= 60 * 24 * 3600 * 1000);
    } 

    const totalLast = last30.length;
    const totalPrev = prev30.length || 1; // avoid division by zero
    ...
    if (deltaEl && now) { // On affiche le delta seulement si on a des dates
      const deltaRounded = Math.round(delta);
      deltaEl.textContent = (deltaRounded > 0 ? "+" : "") + deltaRounded + "% vs prev.";
      deltaEl.className = "kpi-delta " + (deltaRounded >= 0 ? "positive" : "negative");
    } else if (deltaEl) {
        deltaEl.textContent = "";
    }

    const roleCounts = d3.rollup(last30, (v) => v.length, (d) => d._role);
    const bestRole = Array.from(roleCounts.entries()).sort((a, b) => b[1] - a[1])[0];
    ...
    const countTech = (subset) =>
      d3.rollup(
        ...
        (d) => {
          ...
          const found = TRACKED_TECH.filter((t) => Array.from(set).some((s) => s.includes(t)));
  ...
  function aggregateJobsByMonth(dataSubset) {
    ...
    dataSubset.forEach((d) => {
      ...
      if (!monthMap.has(mKey)) {
        monthMap.set(mKey, { month: new Date(mKey), count: 0 });
      }
  ...
  function updateJobVolumeChart() {
    ...
    if (!dataAll.length) return; // No data at all
    ...
    const filtered = aggregateJobsByMonth(filteredJobs.length ? filteredJobs : dataAll);

    // S'il n'y a pas de dates valides, on affiche un message
    ...
    svg.append("path").datum(baseline).attr("fill", "none").attr("stroke", "#4b5563").attr("stroke-width", 2).attr("d", line).attr("opacity", 0.9);
    svg.append("path").datum(filtered).attr("fill", "none").attr("stroke", "#22d3ee").attr("stroke-width", 2.4).attr("stroke-linecap", "round").attr("stroke-linejoin", "round").attr("d", line).attr("opacity", 0.95);

    const legend = svg.append("g").attr("transform", `translate(${width - 170},${margin.top + 4})`);
    const legendItems = [{ label: "All jobs (baseline)", color: "#4b5563" }, { label: "Filtered jobs", color: "#22d3ee" }];

    legendItems.forEach((item, i) => {
      const row = legend.append("g").attr("transform", `translate(0,${i * 16})`);
      row.append("rect").attr("width", 10).attr("height", 10).attr("rx", 2).attr("ry", 2).attr("fill", item.color);
      row.append("text").attr("x", 16).attr("y", 9).text(item.label).attr("fill", "#9ca3af").style("font-size", "10px");
    });
  }
  ...
  function updateRoleDriftChart() {
    ...
    if (!roleData.length) {
      svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").text("No temporal data for this role.");
      return;
    }

    const months = Array.from(d3.rollup(roleData, (v) => v.length, (d) => +d._month).keys()).map((t) => new Date(t)).sort(d3.ascending);
    const maxMonths = 8;
    const selectedMonths = months.length > maxMonths ? months.slice(-maxMonths) : months;

    const records = [];
    const KEY_SKILLS = ["sql", "python", "tableau", "power bi", "powerbi", "excel", "snowflake", "dbt", "airflow", "spark", "aws", "gcp", "azure"];
    ...
    roleData.forEach((job) => {
      ...
      KEY_SKILLS.forEach((skill) => {
        if (Array.from(skillSet).some((s) => s.includes(skill))) {
          records.push({ month: new Date(monthKey), skill });
        }
      });
    });

    if (!records.length) {
      svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").text("No tracked skills found for this role.");
      return;
    }

    const aggregated = d3.rollups(records, (v) => v.length, (d) => d.skill, (d) => +d.month);
    const series = aggregated.map(([skill, entries]) => {
      const map = new Map(entries);
      return { skill, values: selectedMonths.map((m) => ({ month: m, count: map.get(+m) || 0 })) };
    });

    const ranked = series
      .map((s) => ({ ...s, total: d3.sum(s.values, (v) => v.count) }))
      .filter((s) => s.total > 0)
      .sort((a, b) => b.total - a.total)
      .slice(0, 6);

    if (!ranked.length) {
      svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").text("Not enough signal to show evolution.");
      return;
    }

    const x = d3.scaleTime().domain(d3.extent(selectedMonths)).range([margin.left, width - margin.right]);
    const maxCount = d3.max(ranked, (s) => d3.max(s.values, (v) => v.count));
    const y = d3.scaleLinear().domain([0, maxCount || 1]).nice().range([height - margin.bottom, margin.top + 10]);
    const color = d3.scaleOrdinal().domain(ranked.map((d) => d.skill)).range(["#22d3ee", "#a855f7", "#f97316", "#4ade80", "#facc15", "#38bdf8"]);

    const xAxis = d3.axisBottom(x).ticks(selectedMonths.length).tickFormat(d3.timeFormat("%b %y"));
    const yAxis = d3.axisLeft(y).ticks(4).tickSize(-width + margin.left + margin.right);

    svg.append("g").attr("transform", `translate(0,${height - margin.bottom})`).attr("class", "axis").call(xAxis);
    svg.append("g").attr("transform", `translate(${margin.left},0)`).attr("class", "axis").call(yAxis).call((g) => g.selectAll(".tick line").attr("stroke-dasharray", "2,2")).call((g) => g.select(".domain").remove());

    const line = d3.line().x((d) => x(d.month)).y((d) => y(d.count)).curve(d3.curveMonotoneX);

    svg.selectAll(".skill-line").data(ranked).join("g").attr("class", "skill-line").append("path").attr("fill", "none").attr("stroke-width", 2).attr("stroke", (d) => color(d.skill)).attr("d", (d) => line(d.values)).attr("opacity", 0.9);

    ranked.forEach((s) => {
      const cls = s.skill.replace(/\s+/g, "-");
      svg.selectAll(`.point-${cls}`).data(s.values).join("circle").attr("class", `point-${cls}`).attr("cx", (d) => x(d.month)).attr("cy", (d) => y(d.count)).attr("r", 3).attr("fill", color(s.skill))
        .on("mouseenter", (event, dVal) => {
          tooltip.style("opacity", 1).style("left", event.offsetX + 14 + "px").style("top", event.offsetY - 8 + "px").html(`<strong>${s.skill}</strong><br>${d3.timeFormat("%B %Y")(dVal.month)}<br>${dVal.count} offer(s)`);
        })
        .on("mouseleave", () => tooltip.style("opacity", 0));
    });

    const legendItems = legendDiv.selectAll(".legend-item").data(ranked).join("div").attr("class", "legend-item");
    legendItems.append("div").attr("class", "legend-swatch").style("background", (d) => color(d.skill));
    legendItems.append("span").text((d) => `${d.skill} · ${d.total} mention(s)`);
  }

  /* 9. GENERIC DRIFT CHART */
  function genericDriftChart({ idPrefix, accessor, titleAccessor, minCount, maxSeries }) {
    ...
    data.forEach((d) => { if (d._month) monthSet.add(+d._month); });
    const months = Array.from(monthSet).map((t) => new Date(t)).sort(d3.ascending);

    if (!months.length) {
      svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").text("No temporal signal in the current filters.");
      return;
    }
    ...
    data.forEach((d) => {
      ...
      unique.forEach((val) => { if (val) records.push({ month: new Date(monthKey), item: val }); });
    });
    ...
    const aggregated = d3.rollups(records, (v) => v.length, (d) => d.item, (d) => +d.month);
    const series = aggregated.map(([item, entries]) => {
      const map = new Map(entries);
      return { item, values: selectedMonths.map((m) => ({ month: m, count: map.get(+m) || 0 })) };
    });

    const ranked = series.map((s) => ({ ...s, total: d3.sum(s.values, (v) => v.count) })).filter((s) => s.total >= minCount).sort((a, b) => b.total - a.total).slice(0, maxSeries);
    ...
    const maxCount = d3.max(ranked, (s) => d3.max(s.values, (v) => v.count));
    const y = d3.scaleLinear().domain([0, maxCount || 1]).nice().range([height - margin.bottom, margin.top + 10]);
    const color = d3.scaleOrdinal().domain(ranked.map((d) => d.item)).range(["#22d3ee", "#a855f7", "#f97316", "#4ade80", "#facc15", "#38bdf8"]);
    ...
    svg.selectAll(".item-line").data(ranked).join("g").attr("class", "item-line").append("path").attr("fill", "none").attr("stroke-width", 2).attr("stroke", (d) => color(d.item)).attr("d", (d) => line(d.values)).attr("opacity", 0.9);

    ranked.forEach((s) => {
      const cls = s.item.replace(/\s+/g, "-");
      svg.selectAll(`.point-${cls}`).data(s.values).join("circle").attr("class", `point-${cls}`).attr("cx", (d) => x(d.month)).attr("cy", (d) => y(d.count)).attr("r", 3).attr("fill", color(s.item))
        .on("mouseenter", (event, dVal) => {
          tooltip.style("opacity", 1).style("left", event.offsetX + 14 + "px").style("top", event.offsetY - 8 + "px").html(`<strong>${titleAccessor(s.item)}</strong><br>${d3.timeFormat("%B %Y")(dVal.month)}<br>${dVal.count} offer(s)`);
        })
        .on("mouseleave", () => tooltip.style("opacity", 0));
    });

    const legendItems = legendDiv.selectAll(".legend-item").data(ranked).join("div").attr("class", "legend-item");
    legendItems.append("div").attr("class", "legend-swatch").style("background", (d) => color(d.item));
    legendItems.append("span").text((d) => `${titleAccessor(d.item)} · ${d.total} mention(s)`);
  }

  function updateSoftSkillDriftChart() {
    genericDriftChart({ idPrefix: "soft-drift", accessor: (d) => d._softSkills, titleAccessor: (s) => s, minCount: 3, maxSeries: 6 });
  }

  /* 10. MY MARKET WORTH */
  let mmwSelectedSkills = new Set(); // Corrigé (déclaré globalement ici)
  ...
  function initMyMarketWorthModule() {
    ...
    if (expSelect) {
      expSelect.addEventListener("change", () => updateMyMarketWorthChart());
    }
  }
  ...
  function updateWorldMapChart() {
    ...
    const countryCounts = d3.rollups(data, (v) => v.length, (d) => (d._country || "Not specified").trim());
    const countMap = new Map(countryCounts);

    function ensureWorldGeo(callback) {
      if (worldGeo) { callback(); return; }
      if (!worldGeoPromise) {
        worldGeoPromise = d3.json("https://unpkg.com/world-atlas@2/countries-110m.json").then((raw) => { worldGeo = topojson.feature(raw, raw.objects.countries); }).catch((err) => console.error("World map load error:", err));
      }
    ...
    ensureWorldGeo(() => {
      if (!worldGeo) return;
      const projection = d3.geoNaturalEarth1().fitSize([width - margin.left - margin.right, height - margin.top - margin.bottom], worldGeo);
      const path = d3.geoPath(projection);

      svg.append("g").selectAll("path").data(worldGeo.features).join("path").attr("d", path).attr("fill", "#020617").attr("stroke", "#1f2937").attr("stroke-width", 0.6);

      const points = worldGeo.features.map((f) => {
      ...
      if (!points.length) {
        svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").text("No country-level information in the current filters.");
        return;
      }
      ...
      svg.append("g").selectAll("circle").data(points).join("circle").attr("cx", (d) => d.x).attr("cy", (d) => d.y).attr("r", (d) => r(d.count)).attr("fill", "rgba(34,211,238,0.7)").attr("stroke", "#0f172a").attr("stroke-width", 1)
        .on("mouseenter", (event, d) => {
          tooltip.style("opacity", 1).style("left", event.offsetX + 10 + "px").style("top", event.offsetY - 8 + "px").html(`<strong>${d.name}</strong><br>${d.count.toLocaleString()} offer(s)`);
        })
        .on("mouseleave", () => tooltip.style("opacity", 0));
    });
  }
  ...
  function setupChartModal() {
    ...
    document.addEventListener("keydown", (e) => { if (e.key === "Escape" && modalBackdrop && !modalBackdrop.classList.contains("hidden")) hideModal(); });
  }
