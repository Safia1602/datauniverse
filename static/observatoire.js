document.addEventListener("DOMContentLoaded", () => {
  ...
  /* SKILL PARSER ROBUSTE (Même logique que le dashboard) */
  function parseSkillList(value) {
    ...
    // Cas 3 : Chaîne à nettoyer (enlève [], {}, quotes)
    let s = String(value).trim();
    s = s.replace(/^[\s\[\{]+|[\s\]\}]+$/g, ""); // Enlève [ { ] }
    s = s.replace(/'/g, '"'); // Quotes normalisées
  ...
  function rowParser(d) {
    ...
    return {
      ...
      // Méta-données salaire pour filtrage précis
  ...
  fetch(DATA_FILE)
    ...
    .then((raw) => {
      ...
      // 2. Filter invalid dates if necessary
  ...
  function applyFilters() {
    ...
    filteredJobs = allJobs.filter((job) => {
      // 1. Date
      if (minDateFilter && job._date && job._date < minDateFilter) return false;
      ...
      // 3. Salary (Check if Annual & USD if filter active)
      if (minSalary > 0) {
        // Must be USD or $
        const isUSD = job._salaryCurrency === 'USD' || job._salaryCurrency === '$';
        // Must be Annual or Yearly
  ...
  function computeGlobalKPIs() {
    ...
    const prev30 = data.filter((d) => now - d._date > 30 * 24 * 3600 * 1000 && now - d._date <= 60 * 24 * 3600 * 1000);
  ...
  function updateJobVolumeChart() {
    ...
    const xDomain = allDates.length > 0 ? d3.extent(allDates) : [new Date(), new Date(new Date().getTime() + 86400000)];
  ...
  function updateRoleDriftChart() {
    ...
    if (!roleData.length) {
      svg.append("text").attr("x", width / 2).attr("y", height / 2).attr("text-anchor", "middle").attr("fill", "#6b7280").text("No data for this role in the current filters.");
      return;
    }
